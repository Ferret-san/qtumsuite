...
// specified address.
func PayToAddrScript(addr qtumsuite.Address) ([]byte, error) {
	const nilAddrErrStr = "unable to generate payment script for nil address"

	switch addr := addr.(type) {
	case *qtumsuite.AddressPubKeyHash:
		if addr == nil {
	...
	case *qtumsuite.AddressScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
	...
	case *qtumsuite.AddressWitnessPubKeyHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
		return payToWitnessPubKeyHashScript(addr.ScriptAddress())
	case *qtumsuite.AddressWitnessScriptHash:
		if addr == nil {
			return nil, scriptError(ErrUnsupportedAddress,
				nilAddrErrStr)
		}
...
func MultiSigScript(pubkeys []*qtumsuite.AddressPubKey, nrequired int) ([]byte, error) {
...
func ExtractPkScriptAddrs(pkScript []byte, chainParams *chaincfg.Params) (ScriptClass, []qtumsuite.Address, int, error) {
	var addrs []qtumsuite.Address
	...
	case PubKeyHashTy:
		...
		requiredSigs = 1
		addr, err := qtumsuite.NewAddressPubKeyHash(pops[2].data,
			chainParams)
		if err == nil {
	...
	case WitnessV0PubKeyHashTy:
		...
		// Skip the pubkey hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := qtumsuite.NewAddressWitnessPubKeyHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case PubKeyTy:
		...
		requiredSigs = 1
		addr, err := qtumsuite.NewAddressPubKey(pops[0].data, chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case ScriptHashTy:
		...
		requiredSigs = 1
		addr, err := qtumsuite.NewAddressScriptHashFromHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case WitnessV0ScriptHashTy:
		...
		// Skip the script hash if it's invalid for some reason.
		requiredSigs = 1
		addr, err := qtumsuite.NewAddressWitnessScriptHash(pops[1].data,
			chainParams)
		if err == nil {
			addrs = append(addrs, addr)
		}
	...
	case MultiSigTy:
		...
		addrs = make([]qtumsuite.Address, 0, numPubKeys)
		for i := 0; i < numPubKeys; i++ {
			addr, err := qtumsuite.NewAddressPubKey(pops[i+1].data,
				chainParams)
			if err == nil {
				addrs = append(addrs, addr)
			}
