...
// newAddressPubKey returns a new qtumsuite.AddressPubKey from the provided
...
func newAddressPubKey(serializedPubKey []byte) qtumsuite.Address {
	addr, err := qtumsuite.NewAddressPubKey(serializedPubKey,
...
// newAddressPubKeyHash returns a new qtumsuite.AddressPubKeyHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressPubKeyHash(pkHash []byte) qtumsuite.Address {
	addr, err := qtumsuite.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
	if err != nil {
...
// newAddressScriptHash returns a new qtumsuite.AddressScriptHash from the
// provided hash.  It panics if an error occurs.  This is only used in the tests
// as a helper since the only way it can fail is if there is an error in the
// test source code.
func newAddressScriptHash(scriptHash []byte) qtumsuite.Address {
	addr, err := qtumsuite.NewAddressScriptHashFromHash(scriptHash,
		&chaincfg.MainNetParams)
	if err != nil {
...
// bogusAddress implements the qtumsuite.Address interface so the tests can ensure
...
// IsForNet lies blatantly to satisfy the qtumsuite.Address interface.
...
func TestPayToAddrScript(t *testing.T) {
	...
	p2shMain, _ := qtumsuite.NewAddressScriptHashFromHash(hexToBytes("e8c300"+
	...
	p2pkCompressedMain, err := qtumsuite.NewAddressPubKey(hexToBytes("02192d"+
	...
	p2pkCompressed2Main, err := qtumsuite.NewAddressPubKey(hexToBytes("03b0b"+
	...
	p2pkUncompressedMain, err := qtumsuite.NewAddressPubKey(hexToBytes("0411"+
	...
	tests := []struct {
		...
		{(*qtumsuite.AddressPubKeyHash)(nil), "", errUnsupportedAddress},
		{(*qtumsuite.AddressScriptHash)(nil), "", errUnsupportedAddress},
		{(*qtumsuite.AddressPubKey)(nil), "", errUnsupportedAddress},
...
func TestMultiSigScript(t *testing.T) {
	...
	tests := []struct {
		keys      []*qtumsuite.AddressPubKey
		nrequired int
		expected  string
		err       error
	}{
		{
			[]*qtumsuite.AddressPubKey{
				p2pkCompressedMain,
				p2pkCompressed2Main,
			},
			1,
