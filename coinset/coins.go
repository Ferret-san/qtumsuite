...
func (cs *CoinSet) TotalValue() (value qtumsuite.Amount) {
...
func satisfiesTargetValue(targetValue, minChange, totalValue qtumsuite.Amount) bool {
...
type CoinSelector interface {
	CoinSelect(targetValue qtumsuite.Amount, coins []Coin) (Coins, error)
}
...
type MinIndexCoinSelector struct {
	MaxInputs       int
	MinChangeAmount qtumsuite.Amount
}

// CoinSelect will attempt to select coins using the algorithm described
// in the MinIndexCoinSelector struct.
func (s MinIndexCoinSelector) CoinSelect(targetValue qtumsuite.Amount, coins []Coin) (Coins, error) {
	cs := NewCoinSet(nil)
...
func (s MinNumberCoinSelector) CoinSelect(targetValue qtumsuite.Amount, coins []Coin) (Coins, error) {
...
func (s MaxValueAgeCoinSelector) CoinSelect(targetValue qtumsuite.Amount, coins []Coin) (Coins, error) {
	sortedCoins := make([]Coin, 0, len(coins))
	sortedCoins = append(sortedCoins, coins...)
...
func (s MinPriorityCoinSelector) CoinSelect(targetValue qtumsuite.Amount, coins []Coin) (Coins, error) {
...
// qtumsuite.Tx, a specific outpoint index, and the number of confirmations
...
func (c *SimpleCoin) Value() qtumsuite.Amount {
	return qtumsuite.Amount(c.txOut().Value)
}
