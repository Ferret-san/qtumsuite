...
func TestAddresses(t *testing.T) {
	tests := []struct {
		...
		f       func() (qtumsuite.Address, error)
		net     *chaincfg.Params
	}{
		// Positive P2PKH tests.
		{
			name:    "mainnet p2pkh",
			addr:    "QhKqWStf9sV5vSUxjp14xAsSQZxwGSx7Sf",
			encoded: "QhKqWStf9sV5vSUxjp14xAsSQZxwGSx7Sf",
			valid:   true,
			result: qtumsuite.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0xe3, 0x4c, 0xce, 0x70, 0xc8, 0x63, 0x73, 0x27, 0x3e, 0xfc,
					0xc5, 0x4c, 0xe7, 0xd2, 0xa4, 0x91, 0xbb, 0x4a, 0x0e, 0x84}
				return qtumsuite.NewAddressPubKeyHash(pkHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			name:    "mainnet p2pkh 2",
			addr:    "QMxyJXEWoi25SnCm6Gf1ngniZgKcEmysBW",
			encoded: "QMxyJXEWoi25SnCm6Gf1ngniZgKcEmysBW",
			valid:   true,
			result: qtumsuite.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
		...
		},
		{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0x13, 0xc6, 0x0d, 0x8e, 0x68, 0xd7, 0x34, 0x9f, 0x5b, 0x4c,
					0xa3, 0x62, 0xc3, 0x95, 0x4b, 0x15, 0x04, 0x50, 0x61, 0xb1}
				return qtumsuite.NewAddressPubKeyHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		{
			name:    "testnet p2pkh",
			addr:    "qUZanL78jHfrN63LpP5YvsN4FNbXuoveFv",
			encoded: "qUZanL78jHfrN63LpP5YvsN4FNbXuoveFv",
			valid:   true,
			result: qtumsuite.TstAddressPubKeyHash(
				[ripemd160.Size]byte{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0x78, 0xb3, 0x16, 0xa0, 0x86, 0x47, 0xd5, 0xb7, 0x72, 0x83,
					0xe5, 0x12, 0xd3, 0x60, 0x3f, 0x1f, 0x1c, 0x8d, 0xe6, 0x8f}
				return qtumsuite.NewAddressPubKeyHash(pkHash, &chaincfg.TestNet3Params)
			},
		...
		},
		{
			...
			addr:  "QMirQ9bwyQcGVJPwKUgapu5ouK2E2Ey4gY",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		...
		// Positive P2SH tests.
		{
			...
			valid:   true,
			result: qtumsuite.TstAddressScriptHash(
				[ripemd160.Size]byte{
			...
			f: func() (qtumsuite.Address, error) {
				script := []byte{
					...
					0xae}
				return qtumsuite.NewAddressScriptHash(script, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0xEE, 0x34, 0xAC, 0x67, 0x6B, 0xDA, 0xF6, 0xE3, 0x70, 0xC8,
					0xC8, 0x20, 0xB9, 0x48, 0xED, 0xFA, 0xD3, 0xA8, 0x73, 0xD8}
				return qtumsuite.NewAddressScriptHashFromHash(pkHash, &customParams)
			},
			net: &customParams,
		},
		{
			...
			f: func() (qtumsuite.Address, error) {
				hash := []byte{
					0xe8, 0xc3, 0x00, 0xc8, 0x79, 0x86, 0xef, 0xa8, 0x4c, 0x37,
					0xc0, 0x51, 0x99, 0x29, 0x01, 0x9e, 0xf8, 0x6e, 0xb5, 0xb4}
				return qtumsuite.NewAddressScriptHashFromHash(hash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			name:    "testnet p2sh",
			addr:    "maBVd887K4FvEdspV8wrce9oqFA2MVdzU9",
			encoded: "maBVd887K4FvEdspV8wrce9oqFA2MVdzU9",
			valid:   true,
			result: qtumsuite.TstAddressScriptHash(
				[ripemd160.Size]byte{
			...
			f: func() (qtumsuite.Address, error) {
				hash := []byte{
					0xc5, 0x79, 0x34, 0x2c, 0x2c, 0x4c, 0x92, 0x20, 0x20, 0x5e,
					0x2c, 0xdc, 0x28, 0x56, 0x17, 0x04, 0x0c, 0x92, 0x4a, 0x0a}
				return qtumsuite.NewAddressScriptHashFromHash(hash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		...
		// Positive P2PK tests.
		{
			...
			encoded: "QNoFCjam9kMMECHsSkrYaZ7xp7Jy8YdstP",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4},
				qtumsuite.PKFCompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					...
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4}
				return qtumsuite.NewAddressPubKey(serializedPubKey, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			...
			encoded: "QRUGGffBMuZvDMu9x5XstgQ8c2ywSE3X8D",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
		...
		},
		{
			...
			encoded: "QNDaWdjxXzJbt8qug6EHPKRvJcay6KtzB6",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
				...
				qtumsuite.PKFUncompressed, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a, 0x01, 0x6b,
					0x49, 0x84, 0x0f, 0x8c, 0x53, 0xbc, 0x1e, 0xb6, 0x8a, 0x38,
		...
		},
		{
			...
			encoded: "QeB4yAQEN2f8ZGKvkpvjxp3yjr8S9ht7qt",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
				...
				qtumsuite.PKFHybrid, chaincfg.MainNetParams.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					0x06, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
		...
		},
		{
			...
			encoded: "QaZpU4wp9pqRcW5K9w3Y9DPU4AnWqXVJWW",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
		...
		},
		{
			...
			addr:    "02192d74d0cb94344c9569c2e77901573d8d7903c3ebec3a957724895dca52c6b4",
			encoded: "qKkeFU4dAw5fw4wExmWJeKzjqPHTDN4Jf4",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					...
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4},
				qtumsuite.PKFCompressed, chaincfg.TestNet3Params.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					0x02, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
					0xc3, 0xeb, 0xec, 0x3a, 0x95, 0x77, 0x24, 0x89, 0x5d, 0xca,
					0x52, 0xc6, 0xb4}
				return qtumsuite.NewAddressPubKey(serializedPubKey, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
			...
			addr:    "03b0bd634234abbb1ba1e986e884185c61cf43e001f9137f23c2c409273eb16e65",
			encoded: "qNRfKQ93P6JEvEYXU6BdxTGudJxRcaKpet",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x03, 0xb0, 0xbd, 0x63, 0x42, 0x34, 0xab, 0xbb, 0x1b, 0xa1,
		...
		},
		{
			...
			encoded: "qKAyZNDpZB2vb1VHC6t3T6JhKtZTFJh6sQ",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a, 0x01, 0x6b,
					...
					0x8b, 0x64, 0xf9, 0xd4, 0xc0, 0x3f, 0x99, 0x9b, 0x86, 0x43,
					0xf6, 0x56, 0xb4, 0x12, 0xa3},
				qtumsuite.PKFUncompressed, chaincfg.TestNet3Params.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					0x04, 0x11, 0xdb, 0x93, 0xe1, 0xdc, 0xdb, 0x8a, 0x01, 0x6b,
					0x49, 0x84, 0x0f, 0x8c, 0x53, 0xbc, 0x1e, 0xb6, 0x8a, 0x38,
		...
		},
		{
			...
			encoded: "qb8U1tt6PDPTG8yJGqaW2avkm86vELtRS5",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x06, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					...
					0x76, 0x83, 0x01, 0x60, 0xc6, 0xd2, 0xeb, 0x5e, 0x6a, 0x4c,
					0x44, 0xd3, 0x3f, 0x45, 0x3e},
				qtumsuite.PKFHybrid, chaincfg.TestNet3Params.PubKeyHashAddrID),
			f: func() (qtumsuite.Address, error) {
				serializedPubKey := []byte{
					0x06, 0x19, 0x2d, 0x74, 0xd0, 0xcb, 0x94, 0x34, 0x4c, 0x95,
					0x69, 0xc2, 0xe7, 0x79, 0x01, 0x57, 0x3d, 0x8d, 0x79, 0x03,
		...
		},
		{
			...
			encoded: "qXXDWoRgB1ZkKNigfwhJCzGF5SkzuKE56C",
			valid:   true,
			result: qtumsuite.TstAddressPubKey(
				[]byte{
					0x07, 0xb0, 0xbd, 0x63, 0x42, 0x34, 0xab, 0xbb, 0x1b, 0xa1,
		...
		// Segwit address tests.
		{
			...
			addr:    "QC1QW508D6QEJXTDG4Y5R3ZARVARY0C5XW7KQ52AT0",
			encoded: "qc1qw508d6qejxtdg4y5r3zarvary0c5xw7kq52at0",
			valid:   true,
			result: qtumsuite.TstAddressWitnessPubKeyHash(
				0,
				[20]byte{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return qtumsuite.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			name:    "segwit mainnet p2wsh v0",
			addr:    "qc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qtd3g7a",
			encoded: "qc1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qtd3g7a",
			valid:   true,
			result: qtumsuite.TstAddressWitnessScriptHash(
				0,
				[32]byte{
			...
			f: func() (qtumsuite.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return qtumsuite.NewAddressWitnessScriptHash(scriptHash, &chaincfg.MainNetParams)
			},
			net: &chaincfg.MainNetParams,
		},
		{
			name:    "segwit testnet p2wpkh v0",
			addr:    "tq1qw508d6qejxtdg4y5r3zarvary0c5xw7kzztr6f",
			encoded: "tq1qw508d6qejxtdg4y5r3zarvary0c5xw7kzztr6f",
			valid:   true,
			result: qtumsuite.TstAddressWitnessPubKeyHash(
				0,
				[20]byte{
			...
			f: func() (qtumsuite.Address, error) {
				pkHash := []byte{
					0x75, 0x1e, 0x76, 0xe8, 0x19, 0x91, 0x96, 0xd4, 0x54, 0x94,
					0x1c, 0x45, 0xd1, 0xb3, 0xa3, 0x23, 0xf1, 0x43, 0x3b, 0xd6}
				return qtumsuite.NewAddressWitnessPubKeyHash(pkHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
			name:    "segwit testnet p2wsh v0",
			addr:    "tq1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qucc3th",
			encoded: "tq1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3qucc3th",
			valid:   true,
			result: qtumsuite.TstAddressWitnessScriptHash(
				0,
				[32]byte{
			...
			f: func() (qtumsuite.Address, error) {
				scriptHash := []byte{
					...
					0x6c, 0x98, 0x56, 0x78, 0xcd, 0x4d, 0x27, 0xa1,
					0xb8, 0xc6, 0x32, 0x96, 0x04, 0x90, 0x32, 0x62}
				return qtumsuite.NewAddressWitnessScriptHash(scriptHash, &chaincfg.TestNet3Params)
			},
			net: &chaincfg.TestNet3Params,
		},
		{
			name:    "segwit testnet p2wsh witness v0",
			addr:    "tq1qqqqqp399et2xygdj5xreqhjjvcmzhxw4aywxecjdzew6hylgvsesswsl2d",
			encoded: "tq1qqqqqp399et2xygdj5xreqhjjvcmzhxw4aywxecjdzew6hylgvsesswsl2d",
			valid:   true,
			result: qtumsuite.TstAddressWitnessScriptHash(
				0,
				[32]byte{
		...
		// Unsupported witness versions (version 0 only supported at this point)
		{
			...
			addr:  "qc1pw508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7k7grplx",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
		...
		},
		{
			...
			addr:  "qc1zw508d6qejxtdg4y5r3zarvaryvg6kdaj",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		// Invalid segwit addresses
		{
			...
			addr:  "tq1qw508d6qejxtdg4y5r3zarvary0c5xw7kg3g4ty",
			valid: false,
			net:   &chaincfg.TestNet3Params,
		},
		{
			name:  "segwit invalid checksum",
			addr:  "qc1qw508d6qejxtdg4y5r3zarvary0c5xw7kv8f3t5",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "segwit invalid witness version",
			addr:  "QC13W508D6QEJXTDG4Y5R3ZARVARY0C5XW7KN40WF2",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
		...
		{
			name:  "segwit invalid program length",
			addr:  "qc10w508d6qejxtdg4y5r3zarvary0c5xw7kw508d6qejxtdg4y5r3zarvary0c5xw7kw5rljs90",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "segwit invalid program length for witness version 0 (per BIP141)",
			addr:  "QC1QR508D6QEJXTDG4Y5R3ZARVARYV98GJ9P",
			valid: false,
			net:   &chaincfg.MainNetParams,
		},
		{
			name:  "segwit mixed case",
			addr:  "tq1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3q0sL5k7",
			valid: false,
			net:   &chaincfg.TestNet3Params,
		},
		{
			name:  "segwit zero padding of more than 4 bits",
			addr:  "tq1pw508d6qejxtdg4y5r3zarqfsj6c3",
			valid: false,
			net:   &chaincfg.TestNet3Params,
		},
		{
			name:  "segwit non-zero padding in 8-to-5 conversion",
			addr:  "tq1qrp33g0q5c5txsp9arysrx4k6zdkfs4nce4xj0gdcccefvpysxf3pjxtptv",
			valid: false,
			net:   &chaincfg.TestNet3Params,
		},
	}
	...
	for _, test := range tests {
		...
		decoded, err := qtumsuite.DecodeAddress(test.addr, test.net)
		...
		if err == nil {
			...
			case *qtumsuite.AddressPubKeyHash:
				saddr = qtumsuite.TstAddressSAddr(encoded)
			...
			case *qtumsuite.AddressWitnessPubKeyHash:
				saddr = qtumsuite.TstAddressSegwitSAddr(encoded)
			...
			case *qtumsuite.AddressWitnessPubKeyHash:
				...
				expVer := test.result.(*qtumsuite.AddressWitnessPubKeyHash).WitnessVersion()
			...
			case *qtumsuite.AddressWitnessScriptHash:
				...
				expVer := test.result.(*qtumsuite.AddressWitnessScriptHash).WitnessVersion()
				if v := a.WitnessVersion(); v != expVer {
					t.Errorf("%v: witness versions do not match:\n%x != \n%x",
						test.name, expVer, v)
					return
